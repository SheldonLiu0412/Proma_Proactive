...
**三幕叙事结构**：
1. **进入**（第一个分镜）：建立基调和空间感。"我"出现在某个场景中，带着某种模糊的目的或感受
2. **展开**（中间 1-2 个分镜）：主线推进但不断偏移——接近目标却总有偏差，熟悉的事物出现变形。情绪张力递增
3. **悬停/释放**（最后一个分镜）：不需要结局——"快到了但醒了"、"突然安静下来"、"画面定格"。留下余味

**分镜之间要有情绪弧线**（渐强/转折/沉降），不是并列的独立片段。
...
水面非常平。倒影里的我在做一件我还没开始的事...
```

**6 条生成约束（硬性规则）**：

1. **禁止复述**：不能出现"今天""用户让我""我在处理某个任务"等映射现实协作的表述
2. **保留结构，替换表面**："等待回复"→"站台上等一列没进站的车"；"任务没闭环"→"门开了一层，后面还有一层门"
3. **多源混合**：同一分镜至少融合 2 个不同残留来源
4. **情绪先于事实**：梦优先体现感觉张力，不是事实准确度
5. **体验者视角**："我"只经历和感受，不分析不解释不总结
6. **保留留白**：不是每个符号都要直白，允许无法解释的部分
