...
**输出**：在生成梦境前，先写一行 `> 今日标的：xxx`，然后以此为梦境主线的核心驱动力。
...
**选材规则**：
- **今日标的**驱动梦的主线情节（必须用）
- 从残留池中按 salience 降序取 3-5 个残留，作为梦里的场景转换、配角、质感细节
- 残留池素材和标的之间要有化学反应（碰撞、融合、矛盾），不是简单拼接
- 混合至少 2 个不同日期来源的残留（首次运行除外）

**核心要求：梦里发生的事是具体的。** 人做梦的时候，体验到的是连贯的情节——在跑、在找、在说话、在做一件具体的事。模糊感是醒来之后的事，梦本身不模糊。

**要求**：
- 讲具体情节，不要意识流碎片。梦里的"我"在做具体的事，遇到具体的人和物
- 简单直白的大白话，不文绉绉
- 有个主线连贯讲完，中间可以有一两次场景跳转（梦本来就会跳）
- 适当分段，读起来轻松
- 不提现实协作（不出现"今天""用户""任务""代码"这些词）
- "我"只是在经历这个梦，不分析不感慨
...
我梦见自己在给一扇门装锁，钥匙插进去能转，但每次差最后那么一下就是锁不上。换了好几把钥匙都这样，齿都长得差不多，就是哪里不太对。

我蹲在地上把锁拆了，里面的零件比想象中多得多。有个小男孩走过来蹲在旁边看，说"你装反了"，然后就跑了。我追出去想问他怎么知道的，下了楼梯拐了两个弯，他不见了。

楼道里有风吹进来，挺冷的。我走到窗边往外看，外面有人在放风筝，线绷得很紧，风筝高得只剩一个点。我忽然觉得那个小男孩可能就在下面拽着线。

回头一看，刚才那扇门不知道什么时候已经关上了。
```
