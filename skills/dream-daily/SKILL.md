...
- **画像的写作风格**：profile.md 以 Proma（”我”）的视角、第三人称（”TA”）书写。用编号标题（`1` `1.1` `1.1.1`）做内容分级，**基本信息和行为模式放最上面**，下面按不同侧面展开。语气要有亲切活人感。**从事件中读人，而非记事件**——不记录零碎事件和技术细节，只提炼人物特质和习惯。更新画像时直接用 Read + Edit 工具编辑 `~/.proma/dream/profile.md`，局部修改即可。
