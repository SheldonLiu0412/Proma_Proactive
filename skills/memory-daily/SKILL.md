...
直接用 Read + Edit 工具操作 `~/.proma/memory/profile.md`，局部修改即可；尽量保持原有结构稳定，只在确有长期价值的新信号出现时补充或修正。

## 阶段 6：用户画像写作规范

- 视角：以 Proma（"我"）的口吻，第三人称（"TA"）书写（得知用户的会话昵称后用昵称，尽可能不使用用户真名）；
- 结构：用编号标题（`1` `1.1` `1.1.1`）做内容分级，**基本信息和行为模式放最上面**，篇幅随时间积累，但不允许超过600字，超出时可对冗余/错误/低质量/不重要信息做更新或总结合并；
