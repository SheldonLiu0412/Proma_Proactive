...
5. 把 memory-setup skill 复制到当前工作区的 skills 目录（由你确认）：
   mkdir -p "$HOME/.proma/agent-workspaces/<当前工作区slug>（或者可以叫做工作区name，不是会话目录名）/skills/memory-setup" && cp "$PROJECT_DIR/skills/memory-setup/SKILL.md" "$HOME/.proma/agent-workspaces/<当前工作区slug>/skills/memory-setup/SKILL.md"

6. 完成后告诉我，询问是否开始 memory-setup 流程。

注意：
- 不要跳步或是提前询问
- 由于实际工作的目录位置在变化，始终使用上述过程中规定的绝对路径
```
...
- **多操作批量映射**：一个指令可触发多个操作（如"添加纠偏 A 并删除 SOP B"）
- **自行目标匹配**：用自然语言描述即可定位目标（如"删除讲不要催复的那条 correction"）
- 列出完整记忆目录结构（`--list`）
...
├── head.md              # 通用头部
├── memory-search/       # 记忆检索 Skill 组件
│   ├── head.md
│   ├── instruction.md
│   └── complete.md
├── memory-edit/         # 记忆操作 Skill 组件
│   ├── head.md
│   ├── instruction.md
│   └── complete.md
...
├── memory-search.json   # 记忆检索 Skill 配置
├── memory-edit.json     # 记忆操作 Skill 配置
└── memory-agent-guide.json

config/
├── llm-config.json              # LLM API 配置（ Anthropic 兼容格式）
...
├── memory-init-review/
│   └── SKILL.md
├── memory-search/       # 记忆检索 Skill（构建产物）
│   └── SKILL.md
└── memory-edit/         # 记忆操作 Skill（构建产物）
    └── SKILL.md
...
│   ├── memory-search.ts          # 记忆检索脚本（LLM 分析范围 → 精准读取 → LLM 生成答案）
│   ├── memory-edit.ts            # 记忆操作脚本（多操作映射、模糊匹配、操作层重试）
│   └── install-memory-instance.ts # 安装/配置实例
├── utils/
│   ├── llm-client.ts             # Anthropic 兼容 LLM 客户端（支持调用层指数退避重试）
...
`config/llm-config.json` 存储 LLM API 配置，支持 Anthropic 兼容格式（如 MiniMax）：
...
直接修改 `config/llm-config.json` 即可切换配置。
...
| Anthropic 兼容 API | LLM 后端（如 MiniMax） |
