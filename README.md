...
├── head.md              # 通用头部
├── gather-today.md      # 收集今日会话
├── gather-all.md        # 收集全量历史会话
├── load-memory.md       # 加载当前记忆
├── analyze-sessions.md  # 分析会话
├── extract-corrections.md  # 提取行为纠偏
├── update-profile-and-prefs.md  # 更新画像与偏好
...
### 运行时数据（存储在 `~/.proma/memory/`）

```
~/.proma/memory/
├── profile.md                  # 用户画像
├── memory-agent-guide.md       # SubAgent 工作规范（运行时副本）
├── preferences/
│   ├── active.json             # 当前生效的偏好
