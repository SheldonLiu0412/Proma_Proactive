...
重要前提：Agent 每次执行 Bash 都是独立 shell，cd 不会在下条命令里持续有效；所以后续所有命令都要用**绝对路径**，或者用 `&&` 把 `cd <绝对路径>` 和命令串在一行里执行。

步骤如下，请依次执行：

1. 先确定 clone 目标位置（记为 $PROJECT_DIR，必须是绝对路径）：
   - 如果你知道我常用的代码目录（从记忆/历史习惯中判断），优先用那个；
   - 否则按平台默认选：
     · macOS：~/Documents/GitHub/Proma_Proactive
     · Windows：%USERPROFILE%\Documents\GitHub\Proma_Proactive（或我常用的 D: 下某位置）
     · Linux：~/Projects/Proma_Proactive
   - 把你选定的完整绝对路径告诉我后再继续。

2. Clone 仓库（把下面的 $PROJECT_DIR 替换为上一步选定的绝对路径）：
...
5. 把 memory-setup skill 复制到当前工作区的 skills 目录（先告诉我当前工作区的 slug，或帮我查一下）：
   mkdir -p ~/.proma/agent-workspaces/<当前工作区slug>/skills/memory-setup && cp "$PROJECT_DIR/skills/memory-setup/SKILL.md" ~/.proma/agent-workspaces/<当前工作区slug>/skills/memory-setup/SKILL.md
